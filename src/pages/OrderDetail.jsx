...
const OrderDetail = () => {
  ...
  return (
    <div className="container mx-auto py-12">
      <h1 className="text-4xl font-bold mb-8">Захиалгын дэлгэрэнгүй #{order.id}</h1>
      <div className="p-8 bg-white rounded-lg shadow-md">
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div>
            <p><span className="font-semibold">Огноо:</span> {new Date(order.createdAt).toLocaleDateString()}</p>
            <p><span className="font-semibold">Төлөв:</span> {order.status}</p>
          </div>
          <div>
            <p><span className="font-semibold">Нийт дүн:</span> ${parseFloat(order.total).toFixed(2)}</p>
          </div>
        </div>
        <div>
            ...
            {order.products.map(product => (
              <div key={product.id} className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-gray-500">Үнэ: ${parseFloat(product.OrderItem.price).toFixed(2)}</p>
                </div>
              </div>
            ))}
          </div>
